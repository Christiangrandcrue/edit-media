...
export async function runGeneratorJob(jobId, numVideos, profile, callbacks = {}) {
  ...
  for (let i = 1; i <= numVideos; i++) {
    try {
      ...
      // Save video metadata with shots information
      const videoMetadataPath = path.join(JOBS_DIR, jobId, 'videos', `video_${String(i).padStart(4, '0')}_metadata.json`);
      const videoMetadata = {
        video_index: i,
      ...
      fs.writeFileSync(videoMetadataPath, JSON.stringify(videoMetadata, null, 2));
      console.log(`[Generator] Saved metadata for video ${i}: ${result.shots_used.hook_ids.length} hooks, ${result.shots_used.mid_ids.length} mids, ${result.shots_used.cta_ids.length} ctas`);
