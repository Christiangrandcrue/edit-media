...
// DB instance imported from shared database.js
...
router.post('/:id/generate-combinations', async (req, res) => {
  ...
  const { hook_ids, mid_ids, cta_ids, mid_counts } = req.body;

  try {
    // Валидация входных данных
    if (!Array.isArray(hook_ids) || hook_ids.length === 0) {
      return res.status(400).json({ 
        success: false, 
        error: 'hook_ids должен быть непустым массивом' 
      });
    }
    if (!Array.isArray(mid_ids) || mid_ids.length === 0) {
      return res.status(400).json({ 
        success: false, 
        error: 'mid_ids должен быть непустым массивом' 
      });
    }
    if (!Array.isArray(cta_ids) || cta_ids.length === 0) {
      return res.status(400).json({ 
        success: false, 
        error: 'cta_ids должен быть непустым массивом' 
      });
    }
    ...
    const totalCombinations = 
      hook_ids.length * mid_ids.length * cta_ids.length;
    ...
    db.prepare(`
      INSERT INTO combination_jobs (
        job_id, project_id, 
        hook_ids, mid_ids, cta_ids, mid_counts,
        total_combinations, status, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, 'queued', datetime('now'), datetime('now'))
    `).run(
      jobId, 
      projectId, 
      JSON.stringify(hook_ids),
      JSON.stringify(mid_ids),
      JSON.stringify(cta_ids),
      JSON.stringify(mid_counts || [1]),
      totalCombinations
    );
...
router.get('/:id/combination-jobs', async (req, res) => {
  ...
  try {
    const jobs = db.prepare(`
      SELECT 
        job_id, project_id, 
        hook_ids, mid_ids, cta_ids,
        total_combinations, progress, status, error,
    ...
    const jobsWithParsed = jobs.map(job => ({
      ...job,
      hook_ids: JSON.parse(job.hook_ids || '[]'),
      mid_ids: JSON.parse(job.mid_ids || '[]'),
      cta_ids: JSON.parse(job.cta_ids || '[]')
    }));
