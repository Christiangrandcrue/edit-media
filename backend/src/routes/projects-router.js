...
/**
 * POST /api/projects/:project_id/master-videos-archive/download
 * Создать и скачать ZIP архив с выбранными master-видео
 */
router.post('/:project_id/master-videos-archive/download', async (req, res) => {
  ...
  try {
    ...
    if (!Array.isArray(master_ids) || master_ids.length === 0) {
      return res.status(400).json({ 
        error: 'master_ids должен быть непустым массивом' 
      });
    }
    ...
    const videos = db.prepare(`
      SELECT master_id, name, video_path 
      FROM master_videos 
      WHERE master_id IN (${placeholders}) AND project_id = ?
    `).all(...master_ids, projectId);

    if (videos.length === 0) {
      return res.status(404).json({ error: 'Видео не найдены' });
    }

    // Проверить существование файлов
    const existingVideos = videos.filter(v => {
      try {
        return fs.existsSync(v.video_path);
      } catch {
        return false;
      }
    });
    ...
    if (existingVideos.length === 0) {
      return res.status(404).json({ error: 'Файлы не найдены на диске' });
    }

    // archiver imported at top
    const archiveName = `masters_${master_ids.length}_videos_${Date.now()}.zip`;
    
    res.setHeader('Content-Type', 'application/zip');
    res.setHeader('Content-Disposition', `attachment; filename="${archiveName}"`);

    const archive = archiver('zip', {
      zlib: { level: 0 } // Без сжатия (видео уже сжаты)
    });
    ...
    // Добавить видео в архив
    existingVideos.forEach(video => {
      const fileName = `${video.name}.mp4`;
      archive.file(video.video_path, { name: fileName });
    });

    await archive.finalize();

    console.log(`[API] Создан архив: ${archiveName} (${existingVideos.length} видео)`);
